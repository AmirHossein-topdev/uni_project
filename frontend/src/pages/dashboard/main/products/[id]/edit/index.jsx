...
const EditProductDashboard = () => {
    ...
    const fetchProduct = async () => {
      try {
        const res = await axios.get(
          `http://localhost:7000/api/product/single-product/${id}`
        );
        const data = res.data;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.patch(
        `http://localhost:7000/api/product/edit-product/${id}`,
        payload
      );
      if (res.data.success) {
        ...
        setMessage({
          type: "error",
          text: res.data.message || "خطا در بروزرسانی محصول",
        });
      }
    } catch (err) {
      console.error(err);
  ...
  if (loadingCategories || loadingBrands || !product)
  ...
  return (
    <DashboardLayout>
      <div
        dir="rtl"
        className="min-h-screen bg-[#1e2939] text-gray-100 p-4 sm:p-6 lg:p-8 font-vazirmatn"
      >
